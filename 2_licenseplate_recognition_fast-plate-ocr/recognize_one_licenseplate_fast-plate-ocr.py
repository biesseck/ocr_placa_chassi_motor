...
def parse_arguments():
    ...
    parser.add_argument('--vistoria', type=str, required=True, default='C:/Users/Bernardo/GitHub/bot_download_chassi_img/qualit/vistorias_qualit/veiculos_vistoria_laudo_chassi_v2_LABELED/qualit_LABELED/vistorias_qualit_LABELED/vistorias_download_LABELED/Apto_2023-04-22_05-00-33_2023-04-28_05-42-24')
...
if __name__ == "__main__":
    ...
    print(f"Loading vistoria '{args.vistoria}'")
    json_path = glob.glob(os.path.join(args.vistoria, "dados_vistoria*.json").replace('\\','/'))
    assert len(json_path) > 0, f"No JSON file found in {args.vistoria}"
    ...
    if "URL Placa LABELED" in dados_vistoria_corrected and not dados_vistoria_corrected["URL Placa LABELED"] is None and dados_vistoria_corrected["URL Placa LABELED"] != "":
        img_path = os.path.join(args.vistoria, "imgs", dados_vistoria_corrected["URL Placa LABELED"]).replace('\\','/')
